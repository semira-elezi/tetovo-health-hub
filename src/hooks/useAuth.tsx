...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const loadUserData = (userId: string) => {
      Promise.all([fetchProfile(userId), fetchRoles(userId)]).finally(() => {
        setIsLoading(false);
      });
    };
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      (_event, session) => {
        ...
        if (session?.user) {
          setIsLoading(true);
          // Defer Supabase calls to avoid deadlocking the auth listener
  ...
  const isStaff = roles.some((r) => ["admin", "doctor", "nurse", "receptionist"].includes(r));

  return (
    <AuthContext.Provider
      value={{ user, session, profile, roles, isAdmin, isStaff, isLoading, signIn, signUp, signOut }}
    >
      {children}
    </AuthContext.Provider>
  );
